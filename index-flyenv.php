...
const HOSTS_DIR = '/home/jaro/.config/FlyEnv/server/vhost/apache/';
...
switch( $_GET["action"]??'' ){
	...
	case 'phpchecklist':
		the_phpchecklist_info();
		break;

	default:
		$domains = get_domains();
		the_websites($domains);
		break;
...
function get_group_letter( $domain ){
	$pieces = explode('.',$domain);
	$first_level_domain = array_pop($pieces);
	$second_level_domain = array_pop($pieces);
	return $second_level_domain[0]??$first_level_domain[0]??'';
}
...
function display_search_bar(){
	...
	<div class="search-bar">
		<input type="text" id="websiteSearch" placeholder="Search sites..." onkeyup="filterWebsites()" autofocus/>
	</div>
	<script>
		function filterWebsites() {
			var input = document.getElementById('websiteSearch');
			var filter = input.value.toLowerCase();
			var sections = document.getElementsByClassName('section');
			
			for (var i = 0; i < sections.length; i++) {
				var section = sections[i];
				var websites = section.getElementsByClassName('website');
				var header = section.getElementsByClassName('section-header')[0];
				var hasVisibleSite = false;

				for (var j = 0; j < websites.length; j++) {
					var site = websites[j];
					var siteText = site.textContent || site.innerText;
				...
				header.style.display = hasVisibleSite ? '' : 'none';
				section.style.display = hasVisibleSite ? '' : 'none';
			}
		}
	</script>
HTML;
...
function display_header(){
	...
	<div class="info">
		...
		<a href="/index.php?action=phpchecklist">PHP settings checklist</a>
		{$server}
		<br>
	</div>
HTML;
...
function display_websites( $domains ){
	$text_login = '➡️';
	$text_db = '<svg width="15" height="15"><use xlink:href="#icon-database"></use></svg>';
...
HTML;
	...
	foreach( $domains as $letter => $sites ){
		echo <<<HTML
		<div class="section">
			<h3 class="section-header">{$letter}</h3>
			<table class="websites-list">
HTML;
		...
		foreach( $sites as $site ){
			...
			echo <<<HTML
			<tr class="website">
				<td><a href="{$url}" class="site-link">{$site}</a></td>
HTML;

			if( $login ){
				echo <<<HTML
				<td><a href="{$login}" class="site-action">{$text_login}</a></td>
				<td><a href="{$db}" class="site-action">{$text_db}</a></td>
HTML;
			} else {
				echo <<<HTML
				<td></td><td></td>
...
function the_websites( $domains ){
	display_header();
	display_search_bar();
	display_websites( $domains );
}

function the_phpchecklist_info(){
	$settings = [
		...
		[	'settings' => 'opcache.interned_strings_buffer',
			'expected_value' => '64'
		],
		[	'settings' => 'pcre.jit',
			'expected_value' => '1'
		...
		[	'settings' => 'opcache.jit_buffer_size',
			'expected_value' => '0'
		],
	];
	
	$icon_ok = '<span style="color:green">%s</span>';
	$icon_not_good = '<span style="color:red">%s</span>';
...
HTML;
	...
	foreach($settings as $data){
		$value = ini_get($data['settings']);
		
		if( is_bool($value) ){
			$value = $value? 'true' : 'false';
		}
	
		$icon = $value == $data['expected_value'] ? ' ✔' : ' ✖';
		$expected_markup = $value == $data['expected_value'] ? '' : "  expected: {$data['expected_value']}";
		$value_markup = sprintf($value == $data['expected_value'] ? $icon_ok : $icon_not_good, $value) .$icon. $expected_markup;
	
	$html .= <<<HTML
	<div>{$data['settings']} = $value_markup</div>
HTML;
	}
...
function the_extensions_info(){
	...
	<div class="extensions-container">
		<div class="search-bar">
			<input type="text" id="extensionSearch" placeholder="Search extensions..." onkeyup="filterExtensions()" autofocus/>
		</div>
HTML;
...
HTML;
	...
	<script>
		function filterExtensions() {
			var input = document.getElementById('extensionSearch');
			var filter = input.value.toLowerCase();
			var items = document.getElementsByClassName('extension');
